...
function initCitylistEvent() {
    ...
    $("body").on("click", ".citylistinfo .hd span", function () {
        ...
        //alert("切换省市区" + b.index());
        return b.hasClass("cur") ? !1 : c.find(".citylistinfo").show(),find("ul").eq(b.index()).find("a").length < 1 ? !1 : (c.find(".citylistinfo .hd span").removeClass("cur").eq(b.index()).addClass("cur"), void c.find(".citylistinfo ul").hide().eq(b.index()).show())
    })
