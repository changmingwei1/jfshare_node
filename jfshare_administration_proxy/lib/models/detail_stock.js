...
//}
detailStock.prototype.parseProductSku = function(productSku) {
    //0.清除历史数据
    this.dimstocks = null;
    this.SKUResult = {};
    ...
    //未设置sku
    if (productSku["productSkuMap"] == null) {
    ...
    //2.组织sku data集合
    var stockItems = {};
    for (var skuItem in productSku["productSkuMap"]) { //skuItem   1-2:100-103
        var stockKey = this.getSkuPropKey(skuItem);

        //sku item fields
        var stockProp = {};
        stockProp.curPrice = productSku["productSkuMap"][skuItem].curPrice;
        stockProp.orgPrice = productSku["productSkuMap"][skuItem].orgPrice;
        stockProp.sellerClassNum = productSku["productSkuMap"][skuItem].sellerClassNum;
        stockProp.shelf = productSku["productSkuMap"][skuItem].shelf;
    ...
    //打印各个维度的list
    //alert("共这些维度："+this.productSkuObjs["dimsnums"].length);
    //for(var i=0;i<this.productSkuObjs["dims"].length;i++) {
    //    alert(this.productSkuObjs["dims"][i]["d_id"] + "----" + this.productSkuObjs["dims"][i]["d_value"]);
    //}
    //for(var i=0;i<this.productSkuObjs["dimsnums"].length;i++) {
    //    for (var dimsnumMap in this.productSkuObjs["dimsnums"][i]) { //{2:{"s_id":2,"s_value":"红色","s_image":"0.5369955052156001.jpg"}},{3:{"s_id":3,"s_value":"浅黄色","s_image":""}}
    //        alert("第" + i + "维度属性：" + dimsnumMap + "----"  + this.productSkuObjs["dimsnums"][i][dimsnumMap]["s_id"] + "----" + this.productSkuObjs["dimsnums"][i][dimsnumMap]["s_value"] + "----" + this.productSkuObjs["dimsnums"][i][dimsnumMap]["s_image"]); //{2:{"s_id":2,"s_value":"红色","s_image":"0.5369955052156001.jpg"}}
    //    }
    //}

    //this.fillStockSku(stockSku);
}
...
//}
detailStock.prototype.fillStockSku = function(stockSku) {
    if(stockSku == null) {
    ...
    //2.组织stock data集合
    for (var stockItem in stockSku["stockItemMap"]) { //stockItem   1-2:100-103
        if (stockItem.indexOf("-") < 0) {
            continue;
        }
        var stockKey = this.getSkuPropKey(stockItem);
        //sku item stock fields
        this.dimstocks["stockItems"][stockKey].count = stockSku["stockItemMap"][stockItem].count;
        this.dimstocks["stockItems"][stockKey].lockCount = stockSku["stockItemMap"][stockItem].lockCount;
    }
...
detailStock.prototype.getSkuKey = function(skuItem) {
    var temp1 = skuItem.split(':');
    var stockKey = "";
    for (var i=0;i<temp1.length;i++) {
        var temp2 = temp1[i].split('-');
