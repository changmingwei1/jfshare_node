...
router.get('/productAttribute', function(req, res, next) {
    ...
    try{
        ...
        var resContent = {"code":200,"productInfo":{"productAttribute":"[{\"id\":\"1\",\"name\":\"产地\",\"value\":\"中国\"},{\"id\":\"2\",\"name\":\"寿命\",\"value\":\"\"},{\"id\":\"3\",\"name\":\"型号\",\"value\":\"羽绒床垫\"}]"}};
        res.json(resContent);
    }catch(ex) {
        logger.error("get product info error:" + ex);
        result.code = 500;
...
router.get('/querySeller', function(req, res, next){

    logger.info("进入获取卖家信息接口...");
    var resContent = {code:200};
    ...
    try{
        ...
        var args = {};
        args.sellerId = param.sellerId || 1;

        logger.info("It's test______" + JSON.stringify(param));

        Seller.querySeller(args,function(error, data){
            ...
            if(error){
                ...
                resContent.seller = {sellerId:seller.sellerId, sellerName:seller.sellerName};
                resContent.loginLog = data[0].loginLog;
                resContent.value = data[0].value;

                res.json(resContent);
                logger.info("卖家信息响应:" + JSON.stringify(resContent));
            }
        });
    }catch(ex){
        logger.error("获取卖家信息失败，because :" + ex);
        resContent.code = 500;
        resContent.desc = "获取卖家信息失败";
        res.json(resContent);
    }
});
